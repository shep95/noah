...
fn run() -> Result<()> {
    ...
    if args.system_specs {
        let path = app.path();
        let msg = [
            "The `--system-specs` argument is not supported in the noah CLI, only on the noah binary.",
    ...
    anyhow::ensure!(
        ...
        "Dev servers were removed in v0.157.x please upgrade to SSH remoting: https://noah.asherin.com/faq"
    );
...
fn prompt_open_behavior() -> Option<cli::CliBehaviorSetting> {
    ...
    let prompt = format!(
        ...
        console::style("You can change this later in noah settings"),
    );
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
        fn zed_version_string(&self) -> String {
            format!("noah {} – {}", self.version(), self.path().display(),)
        }
