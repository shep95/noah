...
const DARK_THEMES: [&str; 3] = ["noah", "Ayu Dark", "Gruvbox Dark"];
...
fn render_base_keymap_section(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    return v_flex().gap_2().child(Label::new("Base Keymap")).child(
        ToggleButtonGroup::two_rows(
            ...
            [
                ToggleButtonWithIcon::new("noah", IconName::NoahMark, |_, _, cx| {
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = "noah can only allow services like language servers, project settings, and MCP servers to run after you mark a new project as trusted.";

    SwitchField::new(
        "onboarding-auto-trust-worktrees",
        Some("Trust All Projects By Default"),
        Some("Automatically mark all new projects as trusted to unlock all of noah's features".into()),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
            move |&selection, _, cx| {
...
fn render_shepherd_button() -> impl IntoElement {
    AgentSetupButton::new("shepherd-onboarding")
        .icon(
...
fn render_ai_section(cx: &mut App) -> impl IntoElement {
    ...
    v_flex()
        ...
        .child(
            Label::new("shepherd is built in. Add other agents if you want them.")
                .color(Color::Muted),
        )
        .child(grid)
}
