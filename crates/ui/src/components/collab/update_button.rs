...
impl UpdateButton {
    ...
    pub fn checking() -> Self {
        Self::new(IconName::LoadCircle, "Checking for noah Updates…")
            .icon_animate(true)
            .disabled(true)
    }

    pub fn downloading(progress: Option<f32>) -> Self {
        Self::new(IconName::Download, "Downloading noah Update…")
            .progress(progress)
            .disabled(true)
    }

    pub fn installing(version: impl Into<SharedString>) -> Self {
        Self::new(IconName::LoadCircle, "Installing noah Update…")
            .icon_animate(true)
            .tooltip(version)
            .disabled(true)
    }
